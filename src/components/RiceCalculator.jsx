...
const RiceCalculator = () => {
  const [mode, setMode] = useState("people"); // "people" or "rice"
  ...
  return (
    <section id="calculator" className="bg-sectionPink p-32 text-center rounded-2xl text-black shadow-lg">
...
<div className="absolute top-0 right-0 w-64 h-64 opacity-5">
      ...
      <h2 className="text-6xl font-heading text-sectionDarkPurple mt-4">🍣 Rice Calculator 🍚</h2>
      
      <div className="flex justify-center gap-6 my-6">
        <button 
          className={`px-6 py-3 rounded-full text-lg font-body transition-all ${mode === "people" ? "bg-sectionOrange text-white shadow-md scale-105" : "bg-gray-300 text-black"}`}
          onClick={() => setMode("people")}
        >
          By People
        </button>
        <button 
          className={`px-6 py-3 rounded-full text-lg font-body transition-all ${mode === "rice" ? "bg-sectionOrange text-white shadow-md scale-105" : "bg-gray-300 text-black"}`}
      ...
      <input
        ...
        className="p-3 border-2 border-sectionPurple rounded-lg text-black text-lg font-body w-64 text-center shadow-sm"
        value={input}
        onChange={(e) => setInput(e.target.value)}
      />
      <button 
        onClick={calculate} 
        className="mt-4 bg-sectionPurple text-white px-6 py-3 rounded-full text-lg font-body shadow-md hover:scale-105 transition-transform"
      >
        Calculate 🚀
      </button>
      ...
      <div className="mt-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 select-none">
        <div className="p-4 rounded-xl shadow-md border-2 bg-sectionLightPurple text-center transition-transform hover:scale-105 hover:shadow-lg cursor-grab">
          <p>🍚 Rice</p>
          <p className={`text-8.5xl font-body ${textColorClass}`}>{formatNumber(result.riceNeeded)}g</p>
        </div>
        <div className="p-4 rounded-xl shadow-md border-2 bg-sectionLightPurple text-center transition-transform hover:scale-105 hover:shadow-lg cursor-grab">
          <p>💧 Water</p>
          <p className={`text-8.5xl font-body ${textColorClass}`}>{formatNumber(result.water)}ml</p>
        </div>
        <div className="p-4 rounded-xl shadow-md border-2 bg-sectionLightPurple text-center transition-transform hover:scale-105 hover:shadow-lg cursor-grab">
          <p>🍶 Vinegar</p>
          <p className={`text-8.5xl font-body ${textColorClass}`}>{formatNumber(result.vinegar)}ml</p>
        </div>
        <div className="p-4 rounded-xl shadow-md border-2 bg-sectionLightPurple text-center transition-transform hover:scale-105 hover:shadow-lg cursor-grab">
          <p>🍬 Sugar</p>
          <p className={`text-8.5xl font-body ${textColorClass}`}>{formatNumber(result.sugar)}g</p>
        </div>
      </div>
    </section>
  );
